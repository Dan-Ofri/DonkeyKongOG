			...
			while (mario.getLives() > 0 && !winLevel) { // Main game loop
				...
				while (RUNNING) { // Input and gameplay loop
					...
					if (mario.isWon()) { // Check if Mario has won
						validateWin(levelChoice, fileNames.capacity(), board, winLevel, victory);
						break;
					}
...
void Game::validateWin(int levelChoice, int amountOfLevels, Board& board, bool& winLevel, bool& victory)
{
